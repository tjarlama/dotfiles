...
# 	prompt_segment black default " $(TZ='America/Los_angeles' date +%H:%M) PST";
# 	prompt_segment blue black " $(TZ='Europe/Zurich' date +%H:%M) CET";
}
prompt_timer() {
	if [[ -v timer ]]; then
		now=$(date +%s)
...
prompt_context() {
  if [[ "$USERNAME" != "$DEFAULT_USER" || -n "$SSH_CLIENT" ]]; then
    prompt_segment blue black "󰊠 %(!.%{%F{yellow}%}.)%n"
  fi
}
...
prompt_dir() {
  prompt_segment black default '  %1d'
}
